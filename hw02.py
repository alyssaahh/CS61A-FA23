...
def product(n, term):
    ...
    """
    "*** YOUR CODE HERE ***"
    #needs a counter to go through each term 
    #will always start with term(1)
...
def accumulate(merger, start, n, term):
    ...
    """
    "*** YOUR CODE HERE ***"
    total = start #to begin with first term (start)
    count = 1 #keep track up until n 
    while count <= n:
        total = merger(total, term(count))
        count += 1
    return total
...
def product_using_accumulate(n, term):
    ...
    ['Expr', 'Return']
    """
    return accumulate(mul,1,n,term) #bc multiply need to start with 1
...
def funception(func1, begin):
    ...
    def func2 (end):
        if begin >= end:
           ...
           place = begin #need to keep track of where in range 
           product = 1 #total will always be >= 1
           while place < end:
               product *= func1(place)
               place += 1
        return product
