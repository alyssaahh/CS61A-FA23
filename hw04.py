def filter(condition, lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    #don't mutate same list you iterate through 
    copy = list(lst) # copy of list 
    lst [:] = [i for i in copy if condition(i) ]
    #return lst
...
def max_path_sum(t):
    ...
    "*** YOUR CODE HERE ***"
    # use recursion
    # what if other tree just has root 
    if is_leaf(t):
        return label(t)
    else: 
        return max([max_path_sum(b) for b in branches(t)]) + label(t)

    #similar to height question from discussion 5
...
def has_path(t, word):
    ...
    if label(t) != word[0]: #label doesn't match index 0
        return False
    if len(word) == 1: #if the word length is 1, then that is last letter
        return True 
    for b in branches(t):
        if has_path(b, word[1:]): 
            return True
    return False
    #like find_path from discussion 5
